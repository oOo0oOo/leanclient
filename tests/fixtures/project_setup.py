...
def _create_project(path, name, version, use_mathlib=False, force=False):
    ...
    if not os.path.exists(path):
        ...
        subprocess.run(
            f"elan toolchain install leanprover/lean4:{version}",
            shell=True, cwd=path
        )
        subprocess.run(
            f"elan override set leanprover/lean4:{version}",
            shell=True, cwd=path
        )
...
def setup_test_project():
    ...
    _create_project(TEST_ENV_DIR, TEST_PROJECT_NAME, version, use_mathlib=True, force=True)
