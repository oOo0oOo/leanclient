...
def test_wait_for_diagnostics_atomic_check_and_wait():
    ...
    client.opened_files[path] = FileState(uri=uri, content="", complete=False, processing=False, diagnostics_version=-1)
    
    real_cond = client._close_condition
    
    class CheckedCondition:
        def __enter__(self): real_cond.acquire()
        def __exit__(self, *a): real_cond.release()
        def notify_all(self): real_cond.notify_all()
    ...
    errors = []
    def run():
        try: client._wait_for_diagnostics([uri], inactivity_timeout=0.5)
    ...
    t.join(timeout=1.0)
    if errors: pytest.fail(f"Thread failed: {errors[0]}")
