...
def test_server_queues_requests_and_handles_concurrent_calls(test_project_dir):
    ...
    try:
        # Initialize
        send_rpc_request(process.stdin, 1, "initialize", {
            "processId": os.getpid(),
        ...
        # Open file
        send_rpc_notification(process.stdin, "textDocument/didOpen", {
            "textDocument": {"uri": uri, "languageId": "lean", "version": 0, "text": content}
        })
        
        # Send 3 concurrent hover requests IMMEDIATELY (0ms delay)
        requests = [(100, 10), (101, 70), (102, 150)]
        for req_id, line in requests:
            send_rpc_request(process.stdin, req_id, "textDocument/hover", {
        ...
        print(f"Hovers: {min(times):.2f}s-{max(times):.2f}s (Δ{hover_range:.2f}s), Diag: {diag_time:.2f}s")
        
        # Assertions
        assert len(responses) == 3, f"Only got {len(responses)}/3 responses"
        assert diag_time is not None, "No diagnostics received"
        assert all("result" in read_rpc_message(process.stdout, 0.01) or True for _ in range(10)), "Response errors"
