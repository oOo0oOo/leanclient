...
from leanclient.async_single_file_client import AsyncSingleFileClient
...
class TestSingleFileClient(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_method_overlap(self):
        method_client = dir(AsyncLeanLSPClient)
        method_single = dir(AsyncSingleFileClient)
        ...
        missing = set(missing) - set(ok_missing)
        assert not missing, f"Missing methods in AsyncSingleFileClient: {missing}"

    async def test_creation(self):
        # Instantiate a SingleFileClient
        sfc = AsyncSingleFileClient(self.client, TEST_FILE_PATH)
        self.assertEqual(sfc.file_path, TEST_FILE_PATH)
        res = await sfc.get_goal(9, 15)
        assert "⊢" in res["goals"][0]

        await sfc.close_file()  # Just to test the method

        # Create from a client
        sfc2 = self.client.create_file_client(TEST_FILE_PATH)
        self.assertEqual(sfc2.file_path, TEST_FILE_PATH)
        res2 = await sfc.get_goal(9, 15)
        self.assertEqual(res, res2)

    async def test_requests(self):
        sfc = self.client.create_file_client(TEST_FILE_PATH)
        res = []
        res.append(await sfc.get_completions(9, 15))
        res.append(await sfc.get_completion_item_resolve(res[0][0]))
        res.append(await sfc.get_hover(4, 4))
        res.append(await sfc.get_declarations(6, 4))
        res.append(await sfc.get_definitions(1, 29))
        # res.append(await sfc.get_references(9, 24))
        res.append(await sfc.get_type_definitions(1, 36))
        res.append(await sfc.get_document_symbols())
        res.append(await sfc.get_document_highlights(9, 8))
        res.append(await sfc.get_semantic_tokens())
        res.append(await sfc.get_semantic_tokens_range(0, 0, 10, 10))
        res.append(await sfc.get_folding_ranges())
        res.append(await sfc.get_goal(9, 15))
        res.append(await sfc.get_term_goal(9, 15))
        ...
        # assert item["data"]["name"] == "add_zero_custom"
        # inc = await sfc.get_call_hierarchy_incoming(item)
        # out = await sfc.get_call_hierarchy_outgoing(item)
        # assert inc == []
        # assert len(out) >= 1, f"Expected at least 1 outgoing call, got {len(out)}"

        await sfc.update_file([DocumentContentChange("change", (0, 0), (0, 1))])
        # res = await sfc.get_code_actions(0, 0, 10, 10)
        # res = await sfc.get_code_action_resolve({})
